...
class FaceHubCommand(
  ...
  override fun execute() {
    ...
    val v_launch_a = (HUB_HEIGHT.inMeters - SHOOTER_HEIGHT.inMeters) * SHOOTER_ANGLE.cos.pow(2) - SHOOTER_ANGLE.sin * SHOOTER_ANGLE.cos * distanceToHubMag.inMeters
    val v_launch_b = 2 * (HUB_HEIGHT.inMeters - SHOOTER_HEIGHT.inMeters) * SHOOTER_ANGLE.cos * parallelScalar.inMeters - SHOOTER_ANGLE.sin * distanceToHubMag.inMeters * parallelScalar.inMeters
    val v_launch_c = (HUB_HEIGHT.inMeters - SHOOTER_HEIGHT.inMeters) * parallelScalar.inMeters.pow(2) + Constants.Universal.gravity.inMetersPerSecondPerSecond * distanceToHubMag.inMeters.pow(2) / 2.0
    ...
    val launchSpeed = max(
      (-v_launch_b + sqrt(v_launch_b.pow(2) - 4.0 * v_launch_a * v_launch_c)) / (2 * v_launch_a)
      , (-v_launch_b - sqrt(v_launch_b.pow(2) - 4.0 * v_launch_a * v_launch_c)) / (2 * v_launch_a))
      .meters.perSecond + launchSpeedFF
    val launchSpeedZ = launchSpeed * SHOOTER_ANGLE.sin
    val launchSpeedField = launchSpeed * SHOOTER_ANGLE.cos

    // TODO - Apply shooter to this Command and request velocities to it based off the calculated launch speed.
    ...
    // From (6)
    val timeOfFlight = (distanceToHubMag.inMeters / (launchSpeed.inMetersPerSecond * SHOOTER_ANGLE.cos + parallelScalar.inMeters)).seconds
    ...
    CustomLogger.recordOutput("FaceHubCommand/timeOfFlight", timeOfFlight.inSeconds)
